...
function processIndoorSmarthomeMultisensor(data) {
  ...
  let processedData = { isContactDetected: isContactDetected,
                        isMotionDetected: isMotionDetected };
  if(temperature) { processedData.temperature = temperature; };
  if(relativeHumidity) { processedData.relativeHumidity = relativeHumidity; }
  if(illuminance <= 100000) { processedData.illuminance = illuminance; }
  if(acceleration.every(axis => (axis !== null))) {
    processedData.acceleration = acceleration;
  }
